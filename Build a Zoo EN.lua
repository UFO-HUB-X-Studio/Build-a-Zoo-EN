...
-- - ฝั่งซ้าย (Sidebar): สร้าง ScrollingFrame ภายใน left โดยไม่ยุ่งของเดิม
--   (ปล่อย UIListLayout เดิมไว้ แต่เราใช้ ScrollingFrame เป็น "รายการจริง")
-- - ฝั่งขวา (Home): ถ้ายังไม่มีบล็อกเลื่อน จะสร้างให้ พร้อม API + ปุ่ม Create
-- - ทุกชิ้นส่วนมีขอบสีเขียว, เรียงเก่าบน → ใหม่ล่าง, ช็อตคัต + ปุ่ม ▲/▼
----------------------------------------------------------------
do
    ------------------------------------------------------------
    -- ฝั่งซ้าย (Sidebar) : ScrollingFrame + ปุ่มรายการ
    ------------------------------------------------------------
    if not left:FindFirstChild("UFOX_LeftList") then
        ...
        _G.UFOX_AddSideItem = function(text, onclick)
            SSEQ += 1
            local b = makeSideButton(text, onclick, SSEQ) -- เก่าบน → ใหม่ล่าง
            task.defer(function()
        ...
        -- ปุ่มเล็ก ▲/▼ (อยู่ด้านใน sb จะเลื่อนไปพร้อม content)
        local ctrlBar = make("Frame",{
            Parent = sb, BackgroundTransparency=1,
            Size = UDim2.new(1, -0, 0, 0), -- ใช้เป็น anchor สำหรับปุ่ม
            LayoutOrder = -9999
        },{})
        ...
        -- ช็อตคัต Sidebar: Ctrl+PgUp / Ctrl+PgDn
        UIS.InputBegan:Connect(function(i,gp)
            if gp then return end
    ...
    ------------------------------------------------------------
    -- ฝั่งขวา (Home) : ถ้ายังไม่มี list ให้สร้าง + API + ปุ่ม Create
    ------------------------------------------------------------
    ...
    if not homeHasList then
        ...
        -- Toolbar + ปุ่ม Create (เพิ่ม ไม่ไปทับของเดิม)
        ...
        -- ช็อตคัตฝั่งขวา: Ctrl+N = Create, Ctrl+End = ไปท้ายสุด
        UIS.InputBegan:Connect(function(i, gp)
            if gp then return end
